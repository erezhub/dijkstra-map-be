...
| `RateLimiterFilterTest` | GET always passes through untouched; unauthenticated and `ROLE_ADMIN` pass through without calling Redis; under-limit passes, over-limit → 429 with JSON body + `Retry-After` header, boundary (count == limit) still allowed; key format has no service prefix (proves the global/shared quota); Redis throwing fails open; `Retry-After` falls back to the fixed window length when the TTL lookup fails — mocks `StringRedisTemplate` + `RedisScript<Long>` |
